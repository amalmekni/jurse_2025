            ...
            ymd_from_object: function (date) {
                if (date.constructor == Date) {
                    return (
                        date.getFullYear() +
                        "-" +
                        SRE.twodigits(date.getMonth() + 1) +
                        "-" +
                        SRE.twodigits(date.getDate())
                    );
                }
        ...
        utils: {
            is_email: function (email) {
                return (
                    ...
                    email.indexOf("@") > 0 &&
                    email.lastIndexOf(".") > 3 &&
                    email.lastIndexOf(".") > 1 + email.indexOf("@")
                );
            },
        },
    };
    var make_tag = function () {
        ...
        riot.tag2(
            ...
            function (opts) {
                ...
                this.state.images = this.opts.imgs.reduce(
                    function (all, entry) {
                        if (entry.value && entry.key == tag.state.key) {
                            all.push({
                                title: (
                                    entry.attributes.find(function (attr) {
                                        return attr.key == "title";
                                    }) || { value: tag.opts.title }
                                ).value,
                ...
                this.set_active = function () {
                    ...
                    setTimeout(function () {
                        tag.state.filename =
                            tag.state.images[tag.state.current].filename;
                        tag.state.label =
                            tag.state.images[tag.state.current].title;
                        var w = tag.root.getBoundingClientRect().width;
        ...
        riot.tag2(
            "sre-registration-error",
            '<div class="sre-pad-b"> <p class="sre-bold">Errors occured:</p> <div each="{error in parent.state.error}"> &bull; <span class="sre-bold">{error.entity}</span>: {error.message} </div> <br><br> <div class="sre-button-container"> <div class="sre-to-right sre-button sre-prev" onclick="{hideme}">BACK</div> &nbsp; </div> </div>',
            "",
            "",
            function (opts) {
            ...
            function (opts) {
                ...
                this.redirect_now = function (event) {
                    ...
                    document.location.href =
                        "https://livepay.gr/en/payments/ccbs-greece/";
                };
        ...
        riot.tag2(
            ...
            function (opts) {
                ...
                this.set_payment = function (event) {
                    event.preventDefault();
                    tag.payment_method =
                        event.target.getAttribute("data-method");
                };
                ...
                this.submit_form = function () {
                    tag.check_form()
                        .then(tag.parent.pay)
                        .catch(function (msg) {
                            tag.refs.disable_button.style.display =
                                "inline-block";
...
*/
            ...
            function (opts) {
                ...
                this.tools = {
                    select_accommodation_dates: function (dates) {
                        ...
                        tag.state.accommodation.total_stay =
                            tag.SRE.date.days_diff(
                                tag.state.accommodation.date_from,
                                tag.state.accommodation.date_to
                            );
                    ...
                    calculate_accommodation_price: function () {
                        tag.state.accommodation.price =
                            tag.state.accommodation.total_stay *
                            (parseFloat(
                                tag.state.accommodation.selected_room.price
                            ) +
                                parseFloat(
                                    tag.state.accommodation.selected_room
                                        .extra_pax_price *
                                        tag.state.accommodation
                                            .selected_extra_pax
                                ));
                        var conf_from = new Date(
                            tag.conf.date_from.split("-").join("/")
                        );
                        var conf_to = new Date(
                            tag.conf.date_to.split("-").join("/")
                        );
                        ...
                        tag.state.accommodation.needs_confirm =
                            ...
                            tag.state.accommodation.selected_hotel.pivot
                                .strict_dates &&
                            (conf_from > tag.state.accommodation.date_from ||
                                conf_to < tag.state.accommodation.date_to)
                                ? 1
                                : 0;
                    },
                    ...
                    get_hotel_image: function (hotel) {
                        ...
                        return img.length ? img[0].value : null;
                    },
                };
                ...
                this.clicks = {
                    select_dropdown_value: function (dropdown) {
                        return function (event) {
                            event.preventDefault();
                            tag.postdata.ticket.dropdowns[dropdown.admin_name] =
                    ...
                    set_selected_variation: function (variations) {
                        return function (event) {
                            ...
                            tag.postdata.ticket.variation =
                                variations[event.target.value];
                            tag.state.selected_ticket_price =
                                variations[event.target.value].price;
                        };
                    },
                    ...
                    set_active_ticket: function (event) {
                        event.preventDefault();
                        var active = event.item.mainticket.children.filter(
                            function (tt) {
                    ...
                    set_active_hotel: function (event) {
                        ...
                        tag.state.accommodation.selected_hotel =
                            event.item.hotel;
                        tag.state.accommodation.selected_hotel_image =
                            tag.tools.get_hotel_image(
                                tag.state.accommodation.selected_hotel
                            );
                        tag.state.show_hotel_details = true;
                        setTimeout(function () {
                            tag.refs.thescroller.className =
                                "sre-rel-label show";
                        }, 100);
                        setTimeout(function () {
                    ...
                    set_active_room: function (event) {
                        ...
                        if (!tag.state.accommodation.date_from) {
                            tag.state.accommodation.date_from =
                                SRE.date.date_object_from_string(
                                    tag.conf.date_from
                                );
                            tag.state.accommodation.date_to =
                                SRE.date.date_object_from_string(
                        ...
                        tag.state.accommodation.selected_extra_pax =
                    ...
                    accept_hotel_disclaimer: function (event) {
                        event.preventDefault();
                        tag.state.accommodation.extra =
                            tag.refs.hotel_extra.value;
                    ...
                    set_male: function (event) {
                        tag.postdata.user.gender =
                            tag.postdata.user.gender == "Male" ? "" : "Male";
                    },
                    set_female: function (event) {
                        tag.postdata.user.gender =
                            tag.postdata.user.gender == "Female"
                                ? ""
                                : "Female";
                    },
                };
                ...
                this.next_step = function (event) {
                    ...
                    if (tag.state.step == 2) {
                        ...
                        var drop_count =
                            tag.state.selected_mainticket_dropdowns.length;
                        dd(drop_count, tag.postdata.ticket.dropdowns);

                        if (
                            drop_count &&
                            Object.keys(tag.postdata.ticket.dropdowns).length !=
                                drop_count
                        ) {
                ...
                this.on("update", function () {
                    ...
                    tag.state.missing_data =
                        tag.conf.allowZeroCost ||
                        parseFloat(tag.state.final_price) ||
                        tag.state.accommodation.needs_confirm
                            ? 0
                            : 1;
                    ...
                    qwest
                        ...
                        .then(function (h, data) {
                            ...
                            r.billables.map(function (billable) {
                                ...
                                billable.user_count = billable.is_preselected
                                    ? billable.default_count
                                        ? billable.default_count
                                        : 1
                                    : 1;
                            });
                            tag.original_conference = JSON.parse(
                                JSON.stringify(r)
                            );
                            ...
                            tag.state.has_hotels =
                                r.hotels && r.hotels.length > 0;
                            tag.state.has_tickets =
                                r.tickets && r.tickets.length > 0;
                            tag.state.has_billables =
                                r.billables && r.billables.length > 0;
                        ...
                        .catch(function (a, res) {
                            if (res.status == 404) {
                                tag.conf = null;
                                alert(
                                    "Registration for this conference is not available anymore!"
                                );
                                return;
                            }
                            tag.update();
                ...
                this.helpers = {
                    ...
                    update_user_data: function () {
                        ...
                        tag.postdata.billables = tag.conf.billables.reduce(
                            function (acc, b) {
                                if (b.is_selected) {
                                    var hasEF =
                                        b.extrafields != "" &&
                        ...
                        tag.postdata.ticket.ticket_id =
                            tag.state.selected_ticket_id;
                        tag.postdata.ticket.mainticket_id =
                            tag.state.selected_mainticket_id;
                        ...
                        tag.postdata.accommodation = {
                            ...
                            extra_pax:
                                tag.state.accommodation.selected_extra_pax,
                            date_from: tag.state.accommodation.date_from
                                ? tag.SRE.date.ymd_from_object(
                                      tag.state.accommodation.date_from
                                  )
                                : "",
                            date_to: tag.state.accommodation.date_to
                                ? tag.SRE.date.ymd_from_object(
                                      tag.state.accommodation.date_to
                                  )
                                : "",
                    ...
                    calculate_extras_total: function () {
                        tag.state.total_billable_price =
                            tag.conf.billables.reduce(function (acc, blbl) {
                                acc += blbl.is_selected
                                    ? blbl.user_count * Number(blbl.price)
                                    : 0;
                                return acc;
                            }, 0);
                    },
                    check_form: function () {
                        return new Promise(function (resolve, reject) {
                            ...
                            return tag.helpers.ajax_check_email(
                                em.value,
                                resolve,
                                reject
                            );
                    ...
                    create_error_for: function (element) {
                        ...
                        element.addEventListener(
                            "focus",
                            tag.helpers.focus_error_handler
                        );
                    },
                    focus_error_handler: function (event) {
                        setTimeout(function () {
                            event.srcElement.className =
                                event.srcElement.className.replace("error", "");
                            event.srcElement.removeEventListener(
                                "focus",
                                tag.helpers.focus_error_handler
                            );
                        }, 500);
                        ...
                        return qwest
                            ...
                            .then(function (h, data) {
                                ...
                                if (r.result != "valid") {
                                    ...
                                    tag.update();
                                    reject(
                                        "This email does not appear to be valid. Make sure you typed it correctly and then click on VALIDATE EMAIL."
                                    );
                                    return;
                                }
                ...
                this.do_set_active_ticket = function (ticket) {
                    ...
                    var mainticket = tag.conf.tickets.reduce(function (
                        aticket,
                        mt
                    ) {
                        if (mt.id == ticket.id) {
                    ...
                    null);
                    var selectedTicket = mainticket.children.filter(function (
                        aticket
                    ) {
                    ...
                    tag.state.selected_mainticket_dropdowns =
                        ticket.dropdowns || [];

                    tag.state.selected_ticket_id = tag.state.selected_ticket.id;
                    tag.state.selected_ticket_name =
                        tag.state.selected_ticket.name;
                    tag.state.selected_mainticket_name = mainticket.name;
                    tag.state.selected_ticket_price =
                        tag.state.selected_ticket.price;
                    tag.state.selected_ticket_extras = mainticket.extra_fields
                        ? mainticket.extra_fields
                        : [];
                };
        ...
        riot.tag2(
            "sre-user-daterange",
            '<div if="{!state.api_loaded}"> Please wait... loading api </div> <div class="input-field dt-container"> <input type="text" data-ref="range" class="daterange" placeholder="Click to select date"> </div>',
            'sre-user-daterange input[type="text"],[data-is="sre-user-daterange"] input[type="text"]{border:none; border-bottom:1px solid #aaa; outline:none!important; padding:0; margin:10px 0; display:block; width:100%; padding:5px;}',
            "",
            function (opts) {
                ...
                this.state = {
                    api_loaded: false,
                    range_from: this.SRE.date.date_object_from_string(
                        this.opts.fromdate || ""
                    ),
                    range_to: this.SRE.date.date_object_from_string(
                        this.opts.todate || ""
                    ),
                ...
                this.state.max_date = this.opts.maxdate
                    ? this.opts.maxdate
                    : null;
                ...
                this.load_api = function () {
                    return tag.SRE.load_script(
                        "//cdn.jsdelivr.net/npm/flatpickr"
                    )
                        .then(function () {
                            return tag.SRE.load_css(
                                "//cdn.jsdelivr.net/npm/flatpickr/dist/themes/material_green.min.css"
                            );
                        })
                        .catch(function () {
        ...
        riot.tag2(
            ...
            function (opts) {
                ...
                this.set_extra_field = function (event) {
                    ...
                    tag.opts.billables[index].extrafields[
                        event.item.extrafield_index
        ...
        document
            ...
            .appendChild(document.createElement("sre-syn"));
